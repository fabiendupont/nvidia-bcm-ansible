...
DOCUMENTATION = r"""
---
module: monitoring_measurable_health_check
description: ['Manages monitoring_measurable_health_checks']
options:
    state:
      type: str
      choices:
...
RETURN = r"""
---
monitoring_measurable_health_check:
  type: complex
  description: ''
  returned: success
  contains:
...
try:
    ...
    from pythoncm.entity import MonitoringMeasurableHealthCheck
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_measurable_health_check import MonitoringMeasurableHealthCheck_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringMeasurableHealthCheck_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringMeasurableHealthCheck)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringMeasurableHealthCheck, params, commit=not module.check_mode)
            changed = True
    else:
