...
options:
    ...
    newNodeInstallMode:
      description:
      - Installmode to be used by default, for new nodes
      type: str
      required: false
      default: FULL
      choices:
    ...
    managementNetwork:
      description:
      - Determines what network should be used for management traffic. If not set, partition
        setting is used.
      type: str
      required: false
    interactiveUser:
      description:
      - Allow user login on node
      type: str
      required: false
      default: ALWAYS
      choices:
    ...
    allowNetworkingRestart:
      description:
      - Allow nodes to update ifcfg files and restart networking
      type: bool
      required: false
      default: false
    ...
    versionConfigFiles:
      description:
      - Keep old versions of all config files for all nodes in this category
      type: bool
      required: false
      default: false
    bootLoader:
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.category import Category_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=Category_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, Category)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(Category, params, commit=not module.check_mode)
            changed = True
    else:
