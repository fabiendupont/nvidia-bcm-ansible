...
DOCUMENTATION = r"""
---
module: monitoring_image_update_action
description: ['Manages monitoring_image_update_actions']
options:
    state:
      type: str
      choices:
...
RETURN = r"""
---
monitoring_image_update_action:
  type: complex
  description: ''
  returned: success
  contains:
...
try:
    ...
    from pythoncm.entity import MonitoringImageUpdateAction
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_image_update_action import MonitoringImageUpdateAction_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringImageUpdateAction_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringImageUpdateAction)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringImageUpdateAction, params, commit=not module.check_mode)
            changed = True
    else:
