...
---
module: power_shelf
description: ['Manages power_shelfs']
options:
    state:
      type: str
      choices:
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.power_shelf import PowerShelf_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=PowerShelf_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, PowerShelf)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(PowerShelf, params, commit=not module.check_mode)
            changed = True
    else:
