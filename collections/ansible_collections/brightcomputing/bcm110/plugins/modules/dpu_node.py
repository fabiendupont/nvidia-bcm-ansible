...
options:
    ...
    dpuSettings:
      description:
      - Submode containing all DPU node settings
      type: dict
      required: false
      options:
        operation_mode:
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.dpu_node import DPUNode_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=DPUNode_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, DPUNode)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(DPUNode, params, commit=not module.check_mode)
            changed = True
    else:
