...
DOCUMENTATION = r"""
---
module: monitoring_data_producer_gpu
description: ['Manages monitoring_data_producer_gpus']
options:
    state:
      type: str
      choices:
    ...
    updateFreq:
      description:
      - Update frequency of the internal cuda metric sampler
      type: float
      required: false
      default: 10
...
try:
    ...
    from pythoncm.entity import MonitoringDataProducerGPU
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_data_producer_gpu import MonitoringDataProducerGPU_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringDataProducerGPU_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringDataProducerGPU)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringDataProducerGPU, params, commit=not module.check_mode)
            changed = True
    else:
