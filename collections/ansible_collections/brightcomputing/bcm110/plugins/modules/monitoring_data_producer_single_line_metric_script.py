...
DOCUMENTATION = r"""
---
module: monitoring_data_producer_single_line_metric_script
description: ['Manages monitoring_data_producer_single_line_metric_scripts']
options:
    state:
      type: str
      choices:
...
EXAMPLES = r"""
- brightcomputing.bcm110.monitoring_data_producer_single_line_metric_script:
    name: example_metric_1
    description: single line metric running on all nodes
    script: /cm/local/apps/cmd/scripts/metrics/example_sampler.sh
    interval: 120
    consolidator: none
    typeClass: Example
    startupDelay: 0.0
...
- brightcomputing.bcm110.monitoring_data_producer_single_line_metric_script:
    ...
    description: single line metric running on all physical node
    script: /cm/local/apps/cmd/scripts/metrics/example_sampler.sh
    interval: 120
    nodeExecutionFilters_MonitoringResourceExecutionFilter:
...
- brightcomputing.bcm110.monitoring_data_producer_single_line_metric_script:
    name: example_metric_2
    description: single line metric running on the active head and do so for each
      partition
    script: /cm/local/apps/cmd/scripts/metrics/example_sampler.sh
    interval: 120
    nodeExecutionFilters_MonitoringResourceExecutionFilter:
...
RETURN = r"""
---
monitoring_data_producer_single_line_metric_script:
  type: complex
  description: ''
  returned: success
  contains:
...
try:
    ...
    from pythoncm.entity import MonitoringDataProducerSingleLineMetricScript
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_data_producer_single_line_metric_script import MonitoringDataProducerSingleLineMetricScript_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringDataProducerSingleLineMetricScript_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringDataProducerSingleLineMetricScript)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringDataProducerSingleLineMetricScript, params, commit=not module.check_mode)
            changed = True
    else:
