...
DOCUMENTATION = r"""
---
module: monitoring_nvsm_health_dump_action
description: ['Manages monitoring_nvsm_health_dump_actions']
options:
    state:
      type: str
      choices:
...
RETURN = r"""
---
monitoring_nvsm_health_dump_action:
  type: complex
  description: ''
  returned: success
  contains:
...
try:
    ...
    from pythoncm.entity import MonitoringNVSMHealthDumpAction
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_nvsm_health_dump_action import MonitoringNVSMHealthDumpAction_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringNVSMHealthDumpAction_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringNVSMHealthDumpAction)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringNVSMHealthDumpAction, params, commit=not module.check_mode)
            changed = True
    else:
