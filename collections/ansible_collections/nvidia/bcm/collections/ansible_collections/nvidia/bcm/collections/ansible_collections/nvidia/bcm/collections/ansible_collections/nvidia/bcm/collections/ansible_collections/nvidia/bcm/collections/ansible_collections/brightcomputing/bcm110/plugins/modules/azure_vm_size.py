...
DOCUMENTATION = r"""
---
module: azure_vm_size
description: ['Manages azure_vm_sizes']
options:
    state:
      type: str
      choices:
...
try:
    ...
    from pythoncm.entity import AzureVMSize
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.azure_vm_size import AzureVMSize_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=AzureVMSize_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, AzureVMSize)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(AzureVMSize, params, commit=not module.check_mode)
            changed = True
    else:
