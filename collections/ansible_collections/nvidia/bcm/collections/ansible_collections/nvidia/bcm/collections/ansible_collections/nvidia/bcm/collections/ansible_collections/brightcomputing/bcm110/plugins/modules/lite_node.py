...
---
module: lite_node
description: ['Manages lite_nodes']
options:
    state:
      type: str
      choices:
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.lite_node import LiteNode_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=LiteNode_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, LiteNode)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(LiteNode, params, commit=not module.check_mode)
            changed = True
    else:
