...
description: ['Manages azure_locations']
options:
    state:
      type: str
      choices:
...
try:
    ...
    from pythoncm.entity import AzureLocation
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.azure_location import AzureLocation_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=AzureLocation_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, AzureLocation)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(AzureLocation, params, commit=not module.check_mode)
            changed = True
    else:
