...
DOCUMENTATION = r"""
---
module: monitoring_data_producer_network_utilization
description: ['Manages monitoring_data_producer_network_utilizations']
options:
    state:
      type: str
      choices:
...
RETURN = r"""
---
monitoring_data_producer_network_utilization:
  type: complex
  description: ''
  returned: success
  contains:
...
try:
    ...
    from pythoncm.entity import MonitoringDataProducerNetworkUtilization
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_data_producer_network_utilization import MonitoringDataProducerNetworkUtilization_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringDataProducerNetworkUtilization_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringDataProducerNetworkUtilization)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringDataProducerNetworkUtilization, params, commit=not module.check_mode)
            changed = True
    else:
