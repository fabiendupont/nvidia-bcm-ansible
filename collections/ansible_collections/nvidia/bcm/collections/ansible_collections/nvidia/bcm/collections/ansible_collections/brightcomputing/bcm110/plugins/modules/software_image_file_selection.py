...
DOCUMENTATION = r"""
---
module: software_image_file_selection
description: ['Software image file selection']
options:
    state:
      type: str
      choices:
...
try:
    ...
    from pythoncm.entity import SoftwareImageFileSelection
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.software_image_file_selection import SoftwareImageFileSelection_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=SoftwareImageFileSelection_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, SoftwareImageFileSelection)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(SoftwareImageFileSelection, params, commit=not module.check_mode)
            changed = True
    else:
