...
DOCUMENTATION = r"""
---
module: monitoring_data_producer_aggregate_pdu
description: ['Manages monitoring_data_producer_aggregate_pdus']
options:
    state:
      type: str
      choices:
    ...
    excludePDUs:
      description:
      - List of PDUs to exclude from the total
      type: list
      required: false
      default: []
    extraMeasurables:
...
RETURN = r"""
---
monitoring_data_producer_aggregate_pdu:
  type: complex
  description: ''
  returned: success
  contains:
...
try:
    ...
    from pythoncm.entity import MonitoringDataProducerAggregatePDU
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_data_producer_aggregate_pdu import MonitoringDataProducerAggregatePDU_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringDataProducerAggregatePDU_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringDataProducerAggregatePDU)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringDataProducerAggregatePDU, params, commit=not module.check_mode)
            changed = True
    else:
