...
description: ['Manages generic_devices']
options:
    state:
      type: str
      choices:
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.generic_device import GenericDevice_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=GenericDevice_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, GenericDevice)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(GenericDevice, params, commit=not module.check_mode)
            changed = True
    else:
