...
DOCUMENTATION = r"""
---
module: monitoring_data_producer_script
description: ['Manages monitoring_data_producer_scripts']
options:
    state:
      type: str
      choices:
...
EXAMPLES = r"""
- brightcomputing.bcm110.monitoring_data_producer_script:
    name: example_data_producer_1
    description: monitoring collection script running on all nodes
    script: /cm/local/apps/cmd/scripts/collections/example_data_producer.sh
    interval: 120
    consolidator: none
    typeClass: Example
    startupDelay: 0.0
    state: present
- brightcomputing.bcm110.monitoring_data_producer_script:
    name: example_data_producer_2
    description: monitoring collection script running on all physical node
    script: /cm/local/apps/cmd/scripts/collections/example_data_producer.sh
    interval: 120
    nodeExecutionFilters_MonitoringResourceExecutionFilter:
    ...
    state: present
- brightcomputing.bcm110.monitoring_data_producer_script:
    name: example_data_producer_3
    description: monitoring collection script running on the active head and do so
      for each partition
    script: /cm/local/apps/cmd/scripts/collections/example_data_producer.sh
    interval: 120
    nodeExecutionFilters_MonitoringResourceExecutionFilter:
...
RETURN = r"""
---
monitoring_data_producer_script:
  type: complex
  description: ''
  returned: success
  contains:
...
try:
    ...
    from pythoncm.entity import MonitoringDataProducerScript
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_data_producer_script import MonitoringDataProducerScript_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringDataProducerScript_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringDataProducerScript)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringDataProducerScript, params, commit=not module.check_mode)
            changed = True
    else:
