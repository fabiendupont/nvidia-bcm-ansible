...
try:
    ...
    from pythoncm.entity import CertificateRequest
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.certificate_request import CertificateRequest_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=CertificateRequest_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, CertificateRequest)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(CertificateRequest, params, commit=not module.check_mode)
            changed = True
    else:
