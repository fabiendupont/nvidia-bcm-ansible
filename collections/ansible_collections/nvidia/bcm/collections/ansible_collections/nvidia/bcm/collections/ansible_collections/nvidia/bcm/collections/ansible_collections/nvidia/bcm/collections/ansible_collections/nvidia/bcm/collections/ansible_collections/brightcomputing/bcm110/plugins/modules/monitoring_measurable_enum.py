...
DOCUMENTATION = r"""
---
module: monitoring_measurable_enum
description: ['Manages monitoring_measurable_enums']
options:
    state:
      type: str
      choices:
...
try:
    ...
    from pythoncm.entity import MonitoringMeasurableEnum
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_measurable_enum import MonitoringMeasurableEnum_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringMeasurableEnum_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringMeasurableEnum)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringMeasurableEnum, params, commit=not module.check_mode)
            changed = True
    else:
