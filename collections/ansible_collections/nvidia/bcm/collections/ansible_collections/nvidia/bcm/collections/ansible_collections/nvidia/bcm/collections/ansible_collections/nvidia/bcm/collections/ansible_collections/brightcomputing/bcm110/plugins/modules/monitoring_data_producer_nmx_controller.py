...
DOCUMENTATION = r"""
---
module: monitoring_data_producer_nmx_controller
description: ['Manages monitoring_data_producer_nmx_controllers']
options:
    state:
      type: str
      choices:
...
RETURN = r"""
---
monitoring_data_producer_nmx_controller:
  type: complex
  description: ''
  returned: success
  contains:
...
try:
    ...
    from pythoncm.entity import MonitoringDataProducerNMXController
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_data_producer_nmx_controller import MonitoringDataProducerNMXController_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringDataProducerNMXController_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringDataProducerNMXController)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringDataProducerNMXController, params, commit=not module.check_mode)
            changed = True
    else:
