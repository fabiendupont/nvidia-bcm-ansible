...
options:
    ...
    layout:
      description:
      - 'Layout definition for rackview (Format: [|-]x,y   e.g: |8,3 or -2,6)'
      type: str
      required: false
...
chassis:
  ...
  contains:
    ...
    layout:
      type: str
      description: 'Layout definition for rackview (Format: [|-]x,y   e.g: |8,3 or
        -2,6)'
      returned: success
...
try:
    ...
    from pythoncm.entity import Chassis
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.chassis import Chassis_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=Chassis_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, Chassis)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(Chassis, params, commit=not module.check_mode)
            changed = True
    else:
