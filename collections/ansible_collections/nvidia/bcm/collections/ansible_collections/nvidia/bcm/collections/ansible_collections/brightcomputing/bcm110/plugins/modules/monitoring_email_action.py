...
DOCUMENTATION = r"""
---
module: monitoring_email_action
description: ['Manages monitoring_email_actions']
options:
    state:
      type: str
      choices:
    ...
    allAdministrators:
      description:
      - Also send e-mail to all administrator
      type: bool
      required: false
      default: false
    server:
    ...
    info:
      description:
      - Extra information passed in the e-mail
      type: str
      required: false
    timeout:
      description:
    ...
    mergeDelay:
      description:
      - Maximal action delay in order to merge with others
      type: float
      required: false
      default: 0.0
    mergeTrigger:
      description:
      - Merge action from multiple triggers into one
      type: bool
      required: false
      default: false
    mergeMeasurable:
      description:
      - Merge action from multiple measurables into one
      type: bool
      required: false
      default: false
    extra_values:
...
try:
    ...
    from pythoncm.entity import MonitoringEmailAction
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_email_action import MonitoringEmailAction_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringEmailAction_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringEmailAction)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringEmailAction, params, commit=not module.check_mode)
            changed = True
    else:
