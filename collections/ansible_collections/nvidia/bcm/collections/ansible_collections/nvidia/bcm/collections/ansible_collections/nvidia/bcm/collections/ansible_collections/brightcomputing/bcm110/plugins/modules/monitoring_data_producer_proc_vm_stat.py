...
DOCUMENTATION = r"""
---
module: monitoring_data_producer_proc_vm_stat
description: ['Manages monitoring_data_producer_proc_vm_stats']
options:
    state:
      type: str
      choices:
...
RETURN = r"""
---
monitoring_data_producer_proc_vm_stat:
  type: complex
  description: ''
  returned: success
  contains:
...
try:
    ...
    from pythoncm.entity import MonitoringDataProducerProcVMStat
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_data_producer_proc_vm_stat import MonitoringDataProducerProcVMStat_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringDataProducerProcVMStat_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringDataProducerProcVMStat)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringDataProducerProcVMStat, params, commit=not module.check_mode)
            changed = True
    else:
