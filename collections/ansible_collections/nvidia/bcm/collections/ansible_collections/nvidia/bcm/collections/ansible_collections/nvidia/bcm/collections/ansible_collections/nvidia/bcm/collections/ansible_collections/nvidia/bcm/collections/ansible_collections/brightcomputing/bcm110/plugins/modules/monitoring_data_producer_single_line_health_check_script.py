...
DOCUMENTATION = r"""
---
module: monitoring_data_producer_single_line_health_check_script
description: ['Manages monitoring_data_producer_single_line_health_check_scripts']
options:
    state:
      type: str
      choices:
    ...
    runInBash:
      description:
      - Run the script in a bash session
      type: bool
      required: false
      default: false
...
EXAMPLES = r"""
- brightcomputing.bcm110.monitoring_data_producer_single_line_health_check_script:
    name: example_health_checker_1
    description: single health check monitoring running on all nodes
    script: /cm/local/apps/cmd/scripts/healthchecks/example_health_checker.sh
    interval: 120
    consolidator: none
    typeClass: Example
    startupDelay: 0.0
    state: present
- brightcomputing.bcm110.monitoring_data_producer_single_line_health_check_script:
    name: example_health_checker_2
    description: single health check monitoring running on all physical node
    script: /cm/local/apps/cmd/scripts/healthchecks/example_health_checker.sh
    interval: 120
    nodeExecutionFilters_MonitoringResourceExecutionFilter:
    - name: physical_node_filter
      type:
      - physicalNode
    consolidator: none
    typeClass: Example
    ...
    state: present
- brightcomputing.bcm110.monitoring_data_producer_single_line_health_check_script:
    name: example_health_checker_3
    description: single health check monitoring running on the active head and do
...
RETURN = r"""
---
monitoring_data_producer_single_line_health_check_script:
  type: complex
  description: ''
  returned: success
  contains:
...
try:
    ...
    from pythoncm.entity import MonitoringDataProducerSingleLineHealthCheckScript
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_data_producer_single_line_health_check_script import MonitoringDataProducerSingleLineHealthCheckScript_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringDataProducerSingleLineHealthCheckScript_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringDataProducerSingleLineHealthCheckScript)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringDataProducerSingleLineHealthCheckScript, params, commit=not module.check_mode)
            changed = True
    else:
