...
DOCUMENTATION = r"""
---
module: os_cloud_region
description: ['Manages os_cloud_regions']
options:
    state:
      type: str
      choices:
...
try:
    ...
    from pythoncm.entity import OSCloudRegion
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.os_cloud_region import OSCloudRegion_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=OSCloudRegion_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, OSCloudRegion)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(OSCloudRegion, params, commit=not module.check_mode)
            changed = True
    else:
