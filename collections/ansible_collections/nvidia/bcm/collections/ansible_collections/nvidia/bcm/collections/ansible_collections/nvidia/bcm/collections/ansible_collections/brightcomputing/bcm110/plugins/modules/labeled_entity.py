...
DOCUMENTATION = r"""
---
module: labeled_entity
description: ['Manages labeled_entities']
options:
    state:
      type: str
      choices:
    ...
    lastUsedTime:
      description:
      - Time entity was last used
      type: int
      required: false
      default: 0
    permanent:
      description:
      - Do not allow automatic deletion
      type: bool
      required: false
      default: false
...
try:
    ...
    from pythoncm.entity import LabeledEntity
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.labeled_entity import LabeledEntity_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=LabeledEntity_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, LabeledEntity)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(LabeledEntity, params, commit=not module.check_mode)
            changed = True
    else:
