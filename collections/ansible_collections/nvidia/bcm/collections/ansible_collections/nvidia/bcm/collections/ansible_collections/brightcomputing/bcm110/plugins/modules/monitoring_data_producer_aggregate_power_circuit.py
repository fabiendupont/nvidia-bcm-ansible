...
DOCUMENTATION = r"""
---
module: monitoring_data_producer_aggregate_power_circuit
description: ['Manages monitoring_data_producer_aggregate_power_circuits']
options:
    state:
      type: str
      choices:
    ...
    maxSampleAge:
      description:
      - Maximal age of sample to contribute
      type: float
      required: false
      default: 300.0
    excludePowerCircuits:
      description:
      - List of power circuits to exclude from the total
      type: list
      required: false
      default: []
    extraMeasurables:
...
RETURN = r"""
---
monitoring_data_producer_aggregate_power_circuit:
  type: complex
  description: ''
  returned: success
  contains:
...
try:
    ...
    from pythoncm.entity import MonitoringDataProducerAggregatePowerCircuit
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_data_producer_aggregate_power_circuit import MonitoringDataProducerAggregatePowerCircuit_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringDataProducerAggregatePowerCircuit_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringDataProducerAggregatePowerCircuit)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringDataProducerAggregatePowerCircuit, params, commit=not module.check_mode)
            changed = True
    else:
