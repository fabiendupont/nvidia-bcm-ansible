...
DOCUMENTATION = r"""
---
module: monitoring_data_producer_aggregate_power_shelf
description: ['Manages monitoring_data_producer_aggregate_power_shelfs']
options:
    state:
      type: str
      choices:
    ...
    degradedLimit:
      description:
      - Number of PSU that need to be down before the rack is degraded
      type: float
      required: false
      default: 0.9
    criticalLimit:
      description:
      - Number of PSU that need to be down before the rack is critical
      type: float
      required: false
      default: 0.8
    excludePowerShelves:
...
RETURN = r"""
---
monitoring_data_producer_aggregate_power_shelf:
  type: complex
  description: ''
  returned: success
  contains:
...
try:
    ...
    from pythoncm.entity import MonitoringDataProducerAggregatePowerShelf
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_data_producer_aggregate_power_shelf import MonitoringDataProducerAggregatePowerShelf_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringDataProducerAggregatePowerShelf_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringDataProducerAggregatePowerShelf)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringDataProducerAggregatePowerShelf, params, commit=not module.check_mode)
            changed = True
    else:
