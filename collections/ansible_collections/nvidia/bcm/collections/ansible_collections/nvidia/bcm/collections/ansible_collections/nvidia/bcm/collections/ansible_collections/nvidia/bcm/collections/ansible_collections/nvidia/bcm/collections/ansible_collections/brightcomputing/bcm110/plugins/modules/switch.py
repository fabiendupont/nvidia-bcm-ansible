...
options:
    ...
    members:
      description:
      - List of switches belonging to this stack
      type: list
      required: false
      default: []
    controlScript:
      description:
      - Custom control script that provides switch functionality
      type: str
      required: false
    ...
    priority:
      description:
      - Devices on multiple switches will be listed under the switch with the largest
        priority
      type: int
      required: false
      default: 0
    ...
    uplinks:
      description:
      - List of ports connected to other switches
      type: list
      required: false
      default: []
    disablePortDetection:
      description:
      - Disable port detection for this switch
      type: bool
      required: false
      default: false
    disablePortMapping:
      description:
      - Disable port index mapping
      type: bool
      required: false
      default: false
    vlanCacheTime:
      description:
      - Time to cache VLAN information
      type: int
      required: false
      default: 300
    hasClientDaemon:
      description:
      - Switch runs a python cluster manager client daemon
      type: bool
      required: false
      default: false
    vrf:
      description:
      - Force a specific VRF to be used by the cluster manager client daemon
      type: str
      required: false
    ztpSettings:
      description:
    ...
    bootFile:
      description:
      - Boot for Infiniband switches
      type: str
      required: false
    subnetManager:
      description:
      - Indicate the subnet manager is running
      type: bool
      required: false
      default: false
    disableSNMP:
...
switch:
  ...
  contains:
    ...
    priority:
      type: int
      description: Devices on multiple switches will be listed under the switch with
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.switch import Switch_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=Switch_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, Switch)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(Switch, params, commit=not module.check_mode)
            changed = True
    else:
