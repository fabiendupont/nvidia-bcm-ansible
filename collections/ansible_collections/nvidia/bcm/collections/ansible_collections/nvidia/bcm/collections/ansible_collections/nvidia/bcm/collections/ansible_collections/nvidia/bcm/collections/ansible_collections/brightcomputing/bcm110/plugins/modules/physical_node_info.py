...
DOCUMENTATION = r"""
---
module: physical_node_info
description: Query cmdaemon for entity of type PhysicalNode
options:
...
- name: display categories details
  debug:
    ...
    - 'mac: {{item.mac}}'
    - 'number of interfaces: {{item.interfaces|length}}'
    - 'cluster name: {{item.partition.clusterName}}'
    - '{{item.interfaces[0].name}}: {{item.interfaces[0].ip}}@{{ item.interfaces[0].network.name
      }}'
...
def main():
    ...
    results = entity_query.find(PhysicalNode)
    ...
    module.exit_json(physical_nodes=json_ready_result, changed=False)
