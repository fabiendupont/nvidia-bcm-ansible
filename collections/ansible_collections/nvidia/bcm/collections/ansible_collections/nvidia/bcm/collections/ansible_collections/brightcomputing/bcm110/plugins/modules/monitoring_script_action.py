...
DOCUMENTATION = r"""
---
module: monitoring_script_action
description: ['Manages monitoring_script_actions']
options:
    state:
      type: str
      choices:
    ...
    nodeEnvironment:
      description:
      - Pass the node environment to the script
      type: bool
      required: false
      default: false
    runInShell:
...
try:
    ...
    from pythoncm.entity import MonitoringScriptAction
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_script_action import MonitoringScriptAction_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringScriptAction_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringScriptAction)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringScriptAction, params, commit=not module.check_mode)
            changed = True
    else:
