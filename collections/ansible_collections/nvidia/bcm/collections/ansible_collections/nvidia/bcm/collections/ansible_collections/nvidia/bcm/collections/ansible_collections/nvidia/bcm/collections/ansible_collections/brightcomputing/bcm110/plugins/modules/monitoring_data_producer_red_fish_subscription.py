...
DOCUMENTATION = r"""
---
module: monitoring_data_producer_red_fish_subscription
description: ['Manages monitoring_data_producer_red_fish_subscriptions']
options:
    state:
      type: str
      choices:
...
RETURN = r"""
---
monitoring_data_producer_red_fish_subscription:
  type: complex
  description: ''
  returned: success
  contains:
...
try:
    ...
    from pythoncm.entity import MonitoringDataProducerRedFishSubscription
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_data_producer_red_fish_subscription import MonitoringDataProducerRedFishSubscription_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringDataProducerRedFishSubscription_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringDataProducerRedFishSubscription)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringDataProducerRedFishSubscription, params, commit=not module.check_mode)
            changed = True
    else:
