...
DOCUMENTATION = r"""
---
module: standalone_monitored_entity
description: ['Manages standalone_monitored_entities']
options:
    state:
      type: str
      choices:
    ...
    type:
      description:
      - Optional type in case name matches an other entity
      type: str
      required: false
    data:
      description:
      - Data that will be passed to the script enivironment
      type: str
      required: false
    notes:
      description:
...
try:
    ...
    from pythoncm.entity import StandaloneMonitoredEntity
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.standalone_monitored_entity import StandaloneMonitoredEntity_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=StandaloneMonitoredEntity_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, StandaloneMonitoredEntity)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(StandaloneMonitoredEntity, params, commit=not module.check_mode)
            changed = True
    else:
