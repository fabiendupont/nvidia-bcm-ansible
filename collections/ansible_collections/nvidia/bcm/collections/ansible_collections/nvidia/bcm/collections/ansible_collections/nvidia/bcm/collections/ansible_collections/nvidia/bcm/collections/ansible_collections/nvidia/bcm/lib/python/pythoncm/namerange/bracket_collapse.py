...
class BracketCollapse:
    @classmethod
    def collapse(cls, items: Iterable[str]) -> str:
        result = []
        for key, grouper in cls._groups(natsort.natsorted(items)):
    ...
    def _groups(cls, items):
        ...
        return itertools.groupby(natsort.natsorted(split, key=cls.__sort_function), cls.__sort_function)
