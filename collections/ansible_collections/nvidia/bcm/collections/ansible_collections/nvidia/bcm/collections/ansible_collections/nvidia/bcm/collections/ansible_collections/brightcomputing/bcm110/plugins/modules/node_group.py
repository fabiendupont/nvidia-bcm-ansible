...
---
module: node_group
description: ['Manages node_groups']
options:
    state:
      type: str
      choices:
...
EXAMPLES = r"""
- name: create ansible-example node group
  brightcomputing.bcm110.node_group:
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.node_group import NodeGroup_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=NodeGroup_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, NodeGroup)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(NodeGroup, params, commit=not module.check_mode)
            changed = True
    else:
