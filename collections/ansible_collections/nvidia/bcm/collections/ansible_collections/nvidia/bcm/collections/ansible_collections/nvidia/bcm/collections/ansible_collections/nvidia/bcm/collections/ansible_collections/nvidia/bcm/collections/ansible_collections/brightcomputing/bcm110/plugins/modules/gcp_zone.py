...
try:
    ...
    from pythoncm.entity import GCPZone
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.gcp_zone import GCPZone_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=GCPZone_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, GCPZone)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(GCPZone, params, commit=not module.check_mode)
            changed = True
    else:
