...
options:
    ...
    racks:
      description:
      - List of racks powered by this unit
      type: list
      required: false
      default: []
...
try:
    ...
    from pythoncm.entity import PowerCircuit
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.power_circuit import PowerCircuit_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=PowerCircuit_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, PowerCircuit)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(PowerCircuit, params, commit=not module.check_mode)
            changed = True
    else:
