...
DOCUMENTATION = r"""
---
module: ocigpu_memory_cluster
description: ['Manages ocigpu_memory_clusters']
options:
    state:
      type: str
      choices:
    ...
    id:
      description:
      - OCID of this GPU memory cluster
      type: str
      required: false
    gpuMemoryFabricId:
      description:
      - OCID of the GPU memory fabric to use
      type: str
      required: false
...
try:
    ...
    from pythoncm.entity import OCIGPUMemoryCluster
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.ocigpu_memory_cluster import OCIGPUMemoryCluster_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=OCIGPUMemoryCluster_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, OCIGPUMemoryCluster)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(OCIGPUMemoryCluster, params, commit=not module.check_mode)
            changed = True
    else:
