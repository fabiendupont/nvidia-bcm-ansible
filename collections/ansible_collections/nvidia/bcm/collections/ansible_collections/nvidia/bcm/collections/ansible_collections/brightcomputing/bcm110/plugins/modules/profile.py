...
options:
    ...
    nonuser:
      description:
      - True if profile not assignable to users.
      type: bool
      required: false
      default: false
    extra_values:
...
profile:
  ...
  contains:
    ...
    nonuser:
      type: bool
      description: True if profile not assignable to users.
      returned: success
...
try:
    ...
    from pythoncm.entity import Profile
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.profile import Profile_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=Profile_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, Profile)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(Profile, params, commit=not module.check_mode)
            changed = True
    else:
