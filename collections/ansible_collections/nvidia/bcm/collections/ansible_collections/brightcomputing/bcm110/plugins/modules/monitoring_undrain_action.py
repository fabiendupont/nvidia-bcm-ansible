...
DOCUMENTATION = r"""
---
module: monitoring_undrain_action
description: ['Manages monitoring_undrain_actions']
options:
    state:
      type: str
      choices:
...
try:
    ...
    from pythoncm.entity import MonitoringUndrainAction
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_undrain_action import MonitoringUndrainAction_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringUndrainAction_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringUndrainAction)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringUndrainAction, params, commit=not module.check_mode)
            changed = True
    else:
