...
DOCUMENTATION = r"""
---
module: monitoring_data_producer_cm_daemon_state
description: ['Manages monitoring_data_producer_cm_daemon_states']
options:
    state:
      type: str
      choices:
...
RETURN = r"""
---
monitoring_data_producer_cm_daemon_state:
  type: complex
  description: ''
  returned: success
  contains:
...
try:
    ...
    from pythoncm.entity import MonitoringDataProducerCMDaemonState
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_data_producer_cm_daemon_state import MonitoringDataProducerCMDaemonState_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringDataProducerCMDaemonState_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringDataProducerCMDaemonState)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringDataProducerCMDaemonState, params, commit=not module.check_mode)
            changed = True
    else:
