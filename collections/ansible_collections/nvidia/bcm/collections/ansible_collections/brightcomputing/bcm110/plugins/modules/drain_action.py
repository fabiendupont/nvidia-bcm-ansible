...
---
module: drain_action
description: ['Manages drain_actions']
options:
    state:
      type: str
      choices:
    ...
    actions:
      description:
      - Actions to execute after the node has been drained
      type: list
      required: false
      default: []
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.drain_action import DrainAction_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=DrainAction_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, DrainAction)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(DrainAction, params, commit=not module.check_mode)
            changed = True
    else:
