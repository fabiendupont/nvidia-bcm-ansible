...
---
module: oci_shape
description: ['Manages oci_shapes']
options:
    state:
      type: str
      choices:
    ...
    memory:
      description:
      - The amount of operating system memory.
      type: str
      required: false
    disks:
      description:
      - 'AWS: The amount of disks comming with the type. Azure: the maximum amount of
        data disks that can be attached to VMs of this type. OCI: The number of local
        disks available for this shape. GCP: Maximum number of persistent disks.'
      type: str
      required: false
    description:
      description:
    ...
    rdmaPorts:
      ...
      - The number of networking ports available for the remote direct memory access (RDMA)
        network between nodes in a high performance computing (HPC) cluster network. If
        the shape does not support cluster networks, this value is 0.
      type: int
      required: false
      default: 0
    isFlexible:
      description:
      - Whether the shape supports creating flexible instances. A flexible shape is a
        shape that lets you customize the number of OCPUs and the amount of memory when
    ...
    memoryMinPerCpu:
      description:
      - The minimum amount of memory per OCPU available for this shape.
      type: int
      required: false
      default: 0
    memoryMaxPerCpu:
      description:
      - The maximum amount of memory per OCPU available for this shape.
      type: int
      required: false
      default: 0
...
oci_shape:
  ...
  contains:
    ...
    disks:
      type: str
      description: 'AWS: The amount of disks comming with the type. Azure: the maximum
        amount of data disks that can be attached to VMs of this type. OCI: The number
        of local disks available for this shape. GCP: Maximum number of persistent
        disks.     '
      returned: success
    description:
      type: str
    ...
    networkPorts:
      type: int
      description: The number of physical network interface card (NIC) ports available
        for this shape.
      returned: success
    rdmaPorts:
      type: int
      description: The number of networking ports available for the remote direct
        memory access (RDMA) network between nodes in a high performance computing
        (HPC) cluster network. If the shape does not support cluster networks, this
        value is 0.
      returned: success
    isFlexible:
      type: bool
      description: Whether the shape supports creating flexible instances. A flexible
        shape is a shape that lets you customize the number of OCPUs and the amount
        of memory when launching or resizing your instance.
      returned: success
    cpusMin:
      type: int
...
try:
    ...
    from pythoncm.entity import OCIShape
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.oci_shape import OCIShape_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=OCIShape_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, OCIShape)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(OCIShape, params, commit=not module.check_mode)
            changed = True
    else:
