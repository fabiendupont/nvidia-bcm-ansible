...
DOCUMENTATION = r"""
---
module: power_distribution_unit
description: ['Manages power_distribution_units']
options:
    state:
      type: str
      choices:
...
try:
    ...
    from pythoncm.entity import PowerDistributionUnit
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.power_distribution_unit import PowerDistributionUnit_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=PowerDistributionUnit_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, PowerDistributionUnit)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(PowerDistributionUnit, params, commit=not module.check_mode)
            changed = True
    else:
