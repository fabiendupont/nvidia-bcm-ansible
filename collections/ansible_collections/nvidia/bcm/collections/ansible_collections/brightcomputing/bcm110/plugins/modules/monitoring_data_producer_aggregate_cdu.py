...
DOCUMENTATION = r"""
---
module: monitoring_data_producer_aggregate_cdu
description: ['Manages monitoring_data_producer_aggregate_cdus']
options:
    state:
      type: str
      choices:
    ...
    maxSampleAge:
      description:
      - Maximal age of node sample to contribute
      type: float
      required: false
      default: 300.0
    excludeCDUs:
      description:
      - List of CDUs to exclude from the total
      type: list
      required: false
      default: []
    extraMeasurables:
...
RETURN = r"""
---
monitoring_data_producer_aggregate_cdu:
  type: complex
  description: ''
  returned: success
  contains:
...
try:
    ...
    from pythoncm.entity import MonitoringDataProducerAggregateCDU
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_data_producer_aggregate_cdu import MonitoringDataProducerAggregateCDU_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringDataProducerAggregateCDU_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringDataProducerAggregateCDU)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringDataProducerAggregateCDU, params, commit=not module.check_mode)
            changed = True
    else:
