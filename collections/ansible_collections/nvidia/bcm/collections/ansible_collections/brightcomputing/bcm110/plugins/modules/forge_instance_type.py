...
DOCUMENTATION = r"""
---
module: forge_instance_type
description: ['Manages forge_instance_types']
options:
    state:
      type: str
      choices:
...
try:
    ...
    from pythoncm.entity import ForgeInstanceType
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.forge_instance_type import ForgeInstanceType_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=ForgeInstanceType_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, ForgeInstanceType)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(ForgeInstanceType, params, commit=not module.check_mode)
            changed = True
    else:
