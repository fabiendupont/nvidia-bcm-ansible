...
DOCUMENTATION = r"""
---
module: monitoring_event_action
description: ['Manages monitoring_event_actions']
options:
    state:
      type: str
      choices:
    ...
    profiles:
      description:
      - Inform all sessions with the specified profile, none is all
      type: list
      required: false
      default: []
    userNames:
      description:
      - Inform all sessions with the specified user names, none is all
      type: list
      required: false
      default: []
    mergeDelay:
...
try:
    ...
    from pythoncm.entity import MonitoringEventAction
except ImportError:
    HAS_PYTHONCM = False
...
from ansible_collections.brightcomputing.bcm110.plugins.module_utils.argspecs.monitoring_event_action import MonitoringEventAction_ArgSpec
...
def main():
    ...
    module = BaseModule(
        argument_spec=MonitoringEventAction_ArgSpec.argument_spec,
    ...
    entity = entity_manager.lookup_entity(params, MonitoringEventAction)
    ...
    if not entity:
        if desired_state == "present":
            entity, res = entity_manager.create_resource(MonitoringEventAction, params, commit=not module.check_mode)
            changed = True
    else:
